...
impl FrontDefinition {
    pub fn convert(&self, parent: &str, _factor: f32) -> Result<FrontStyle> {
        Ok(match self {
            FrontDefinition::None => FrontStyle::None,
            FrontDefinition::One(door_id) => FrontStyle::One(*door_id),
            FrontDefinition::Repeat { step, door_id } => FrontStyle::new_repeat(*step, *door_id)
...
mod tests {
    ...
    fn test_convert_none() {
        assert_eq!(
            FrontDefinition::None.convert("test", 1.0).unwrap(),
            FrontStyle::None
        );
    }
    ...
    fn test_convert_one() {
        assert_eq!(
            FrontDefinition::One(42).convert("test", 1.0).unwrap(),
            FrontStyle::One(42)
        );
    }
    ...
    fn test_convert_repeat() {
        ...
        assert_eq!(definition.convert("test", 1.0).unwrap(), style);
    }
